...
@HiltViewModel
class TenantsUnPaidBillsViewModel @Inject constructor(
    ...
    private val _upPaidBillsUiState = MutableSharedFlow<NetworkResult<ListOfBillsResponse>>()
    val upPaidBillsUiState = _upPaidBillsUiState.asSharedFlow()

    fun getUnPaidBills(
    ) {
        viewModelScope.launch {
            tenantsPropertiesRepo.getUpcomingBills(request).collectLatest { _upPaidBillsUiState.emit(it)}
        }
    }
}